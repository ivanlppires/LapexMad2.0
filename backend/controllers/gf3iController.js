...
export const getIntermediateData = async (req, res) => {
  ...
  try {
    ...
    const estadosDestinatarioArray = ensureArray(estadosDestinatario);
    ...
    if (municipiosRemetenteArray.length > 0 || estadosDestinatario.length > 0) {
      query += `, m_remetente.nome AS nomeMunicipioRemetente`;
      groupByFields.push('nomeMunicipioRemetente');

      query += `, es_destinatario.nome AS estadoDestinatario`;
      groupByFields.push('estadoDestinatario');
      query += `, es_destinatario.uf AS ufEstadoDestinatario`;
      groupByFields.push('ufEstadoDestinatario');
    }
    ...
    query += `
      ...
      INNER JOIN 
        estado es_destinatario ON es_destinatario.id = m_destinatario.estado_id
      WHERE 
        pr.residuo = 0 -- Filtrar por produtos NÃO Residuais
    `;
